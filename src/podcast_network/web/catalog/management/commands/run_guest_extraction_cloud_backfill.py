...
from podcast_network.web.catalog.models import Episode, EpisodeGuestExtraction, ExtractionRun

TERMINAL_BATCH_STATUSES = {"completed", "failed", "expired", "cancelled"}


class Command(BaseCommand):
    help = "Run a cloud-resident, self-coordinating OpenAI Batch API guest backfill."
    ...
    def handle(self, *args: object, **options: object) -> None:
        ...
        while True:
            ...
            remaining = count_remaining_episodes(
                model=model,
                prompt_version=prompt_version,
            )
            self.stdout.write(f"Remaining episodes needing first pass: {remaining}.")
            if remaining == 0:
                self.stdout.write(self.style.SUCCESS("Guest extraction backfill complete."))
                return
            ...
            if not episodes:
                self.stdout.write(self.style.SUCCESS("No selectable episodes remain."))
                return
    ...
    def sync_active_runs(
        ...
        *,
        ...
        while True:
            ...
            if not wait_for_terminal or pending == 0:
                return
            if time.monotonic() - started > max_runtime_seconds:
                self.stdout.write(
                    self.style.WARNING(
    ...
    def submit_batch(
        ...
        *,
        ...
        jsonl_path = (
            output_dir
            / f"guest_extraction_{model}_{timestamp}_wave_{wave:04d}_"
        ...
        try:
            ...
            raise CommandError(
                f"OpenAI batch submission failed for wave {wave} "
...
def count_remaining_episodes(*, model: str, prompt_version: str) -> int:
    ...
    queryset = (
        ...
        .exclude(Exists(successful_extraction))
